...
export function sm2(params) {
  ...
  if (quality === 1) { // 1 = again
    newRepetitions = 0;
    newInterval = 1;
  } else if (quality === 2) { // 2 = hard
    // Don't fully reset repetitions, but interval grows very slowly
    newRepetitions = Math.max(1, repetitions);
    newInterval = Math.max(1, Math.round(interval * 1.2));
  } else if (quality === 3) { // 3 = good
    if (newRepetitions === 0) {
      newInterval = 1;
